exports.init = function(SARAH,config) {
  ...
  var process=ClaraIANircmd + ' filldelete "' + dir + '\\minuteur\\memoire\\*.*"'﻿
...
//si vide alors lecture
 if(reponse==""){
            ...
            console.log(files)
                ...
                if(file.length==0){
                    ClaraIASpeech("pas de minuteur|tu est sur que tu avais programmé un minuteur ?|j'ai rien en mémoire")
                }  
            ...
            ClaraIASpeech("minuteur "+files+" il reste "+Math.floor(tempsfin)+ " minutes " + Math.floor(((tempsfin-Math.floor(tempsfin))*60)) + " secondes ")
               }                     
            }//fin try
            catch(err){ClaraIASpeech("je n'ai pas compris");return };//
 return 
}//fin if reponse
...
ClaraIASpeech("je lance le minuteur "+reponse1+"|si j'y pense|je vais essayer de m'en souvenir|tu m'en veux pas si j'oublie")
...
tim=function(reponse1,tempsreveil){
  console.log("lancement minuteur")
    ...
    setTimeout(function() {
      ...
      ClaraIASpeech("fin du minuteur "+reponse1)
              
                var path = require('path');
                var dir = path.resolve('%CD%', './plugins/').replace('\\%CD%', '');
                var process=ClaraIANircmd + ' filldelete "' + dir + '\\minuteur\\memoire\\'+reponse1+'.json"'﻿
                console.log(process)
                var exec = require('child_process').exec;                       
                exec(process)
